...
<html>
    ...
    <body>
        <p>Jobs in Mymensingh</p>
        <?php
                $db_handle=mysqli_connect("localhost","root","","dreamjob") or die("Could not connect");
                $sql11="select * from jobnews where region='Mymensingh'";
                $result11=mysqli_query($db_handle,$sql11) or die("Could not execute");
                while($row11=mysqli_fetch_array($result11))
                {
                    $id=$row11['id'];
                    $logo=$row11['logo'];
                    $institutionname=$row11['institutionname'];
                    $post=$row11['post'];
                    $eligibility=$row11['eligibility'];
                    $experience=$row11['experience'];
                    $salary=$row11['salary'];
                    $lastdatetoapply=$row11['lastdatetoapply'];
                    $region=$row11['region'];
