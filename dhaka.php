...
<html>
    ...
    <body>
        ...
        <?php
                ...
                $sql5="select * from jobnews where region='Dhaka'";
                $result5=mysqli_query($db_handle,$sql5) or die("Could not execute");
                while($row5=mysqli_fetch_array($result5))
                {
                    $id=$row5['id'];
                    $logo=$row5['logo'];
                    $institutionname=$row5['institutionname'];
                    $post=$row5['post'];
                    $eligibility=$row5['eligibility'];
                    $experience=$row5['experience'];
                    $salary=$row5['salary'];
                    $lastdatetoapply=$row5['lastdatetoapply'];
                    $region=$row5['region'];
